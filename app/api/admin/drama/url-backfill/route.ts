...
function extractOfficialFromDetail(html: string): { title: string | null; officialUrl: string | null } {
  // 1. タイトル: id="drama-title" の h1 を最優先、ダメなら <title> タグから
  let title: string | null = null;

  const dramaTitleMatch = html.match(/<h1[^>]*?id=["']drama-title["'][^>]*>([\s\S]*?)<\/h1>/i);
  if (dramaTitleMatch) {
    title = dramaTitleMatch[1].replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim();
  }

  if (!title) {
    // フォールバック: drama-hero__title クラスの h1
    const dramaHeroMatch = html.match(/<h1[^>]*?class=["'][^"']*?drama-hero__title[^"']*?["'][^>]*>([\s\S]*?)<\/h1>/i);
    if (dramaHeroMatch) {
      title = dramaHeroMatch[1].replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim();
    }
  }

  if (!title) {
    // 最終フォールバック: <title> タグから「ドラマ名 | クランクイン!」形式の前半を取る
